...
def generate_names(char, num):
    ...
    female_names = [(name, 'male') for name in names.words('male.txt') if name.startswith(char)]
    male_names = [(name, 'female') for name in names.words('female.txt') if name.startswith(char)]
